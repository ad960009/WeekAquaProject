...
> [!WARNING]
> **⚠️ 주의사항 (Disclaimer & Hardware Notice)**  
> 1. 본 프로젝트는 안드로이드 공식 앱(`com.weekled.weekaquas`) 역공학 분석을 기반으로 작성된 **연구 및 시험용 소프트웨어**입니다. 실제 실물 디바이스에서의 완전한 동작 및 안전성을 보장하지 않으며, 본 프로그램 또는 명세서의 이용으로 인해 발생하는 기기 이상이나 데이터 손실에 대해 일체의 책임을 지지 않습니다.
> 2. **Live 모드(Mode 1) 불안정성**: WeekAqua 조명 MCU는 실시간 라이브 수동 제어(Mode 1) 시 내부 타이머(`FEF9`/`FEEF`)와 결합되어 작동하므로, 타이머 창이 24시간(`00:00~24:00`)으로 열려 있지 않거나 펌웨어 버전에 따라 색상 변경 시 조명이 일시 소등되는 등 동작이 불안정할 수 있습니다. 가장 안정적인 운용을 위해서는 **다중 슬롯 하드웨어 스케줄(Mode 2)** 또는 **HA 동적 스케줄러** 사용을 권장합니다.
...
- 📅 **5-Slot / 8-Slot / 12-Slot Ramp-up/down 고급 다중 스케줄 에디터 (Advanced Schedule Mode)**
  ...
  - **자연스러운 대칭형 일주기 강도 곡선 기본 탑재**:
    - **5-Slot**: `[25% -> 70% -> 100% -> 35% -> 0%]` (3번 단일 피크)
    - **8-Slot (M800 Pro 등)**: `[20% -> 50% -> 75% -> 100% -> 75% -> 50% -> 20% -> 0%]` (4번 단일 피크)
    - **12-Slot**: `[15% -> 30% -> 50% -> 70% -> 85% -> 100% -> 85% -> 70% -> 50% -> 30% -> 15% -> 0%]` (6번 단일 피크)
  - **자정 24:00 (`0x24 0x00`) 무암전(Gapless) 연속 점등 및 자정 넘김 자동 분할** (18:00 ~ 02:00 등 야간 점등 시 자정 직전 슬롯 24:00 종료 ➡️ 익일 00:00 시작 바통 터치)
